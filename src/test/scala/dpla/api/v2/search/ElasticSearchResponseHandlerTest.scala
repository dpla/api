...
class ElasticSearchResponseHandlerTest extends AnyWordSpec with Matchers
  with BeforeAndAfterAll with AsyncTestSuite {
  ...
  "response processor" should {
    "handle successful response" in {
      ...
      val response: ElasticSearchSuccess =
        probe.expectMessageType[ElasticSearchSuccess]
    ...
    "handle http error" in {
      ...
      responseHandler ! ProcessElasticSearchResponse(httpResponse, probe.ref)
      val response: ElasticSearchHttpError =
        probe.expectMessageType[ElasticSearchHttpError]
    ...
    "handle unreachable endpoint" in {
      ...
      responseHandler ! ProcessElasticSearchResponse(httpResponse, probe.ref)
      probe.expectMessage(ElasticSearchResponseFailure)
    }
  }
}
